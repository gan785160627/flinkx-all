        ...
        if(metaColumns.size() == 1 && ConstantValue.STAR_SYMBOL.equals(metaColumns.get(0).getName())){
            ...
            for (int i = 0; i < metaColumns.size(); i++) {
                ...
                if(metaColumn.getName() != null){
                    if (metaColumn.getName().contains(".")) {
                        List<String> keys = Splitter.on(".").splitToList(metaColumn.getName());
                        value = doc.getEmbedded(keys, metaColumn.getValue() == null ? "default" : metaColumn.getValue());
                    } else if (metaColumn.getName().contains("[") && metaColumn.getName().contains("]")) {
                        List<String> keyAndIndex = Splitter.on(CharMatcher.anyOf("[]")).splitToList(metaColumn.getName());
                        String key = keyAndIndex.get(0);
                        int idx = Integer.parseInt(keyAndIndex.get(1));
                        List<String> list = doc.getList(key, String.class, Lists.newArrayList());
                        if (idx < list.size()) {
                            value = list.get(idx);
                        } else {
                            value = "default";
